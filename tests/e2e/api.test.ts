...
describe("API Integration Tests", () => {
  ...
  describe("Users API", () => {
    ...
    it("should not create user with duplicate email", async () => {
      ...
      const userData = { name: "Duplicate User", email: uniqueEmail, phone: "03-1234-5678" };
      await request(app).post("/api/v1/users").send(userData).expect(201);
      const duplicateData = { name: "Duplicate User 2", email: uniqueEmail, phone: "03-9999-9999" };
      const response = await request(app).post("/api/v1/users").send(duplicateData).expect(400);
  ...
  describe("Debug API", () => {
    it("should get all users (debug)", async () => {
      const response = await request(app).get("/api/v1/debug/users").expect(200);
      expect(response.body.success).toBe(true);
      expect(Array.isArray(response.body.data)).toBe(true);
    });

    it("should get all subscriptions (debug)", async () => {
      const response = await request(app).get("/api/v1/debug/subscriptions").expect(200);
      expect(response.body.success).toBe(true);
      expect(Array.isArray(response.body.data)).toBe(true);
    });
  });
